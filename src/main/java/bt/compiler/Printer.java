...
public class Printer {
	...
	static String funcName(byte []bytes, int end) {
		...
		case OpCode.Get_Creation_Timestamp    : // 0x0301; // EXT_FUN_RET       sets @addr to the timestamp of the AT creation block
			return "Get_Creation_Timestamp";
		case OpCode.Get_Last_Block_Timestamp  : // 0x0302; // EXT_FUN_RET       sets @addr to the timestamp of the previous block
		...
		case OpCode.A_To_Tx_After_Timestamp   : // 0x0304; // EXT_FUN_DAT       sets A to tx hash of the first tx after $addr timestamp
		...
		case OpCode.Get_Timestamp_For_Tx_In_A : // 0x0307; // EXT_FUN_RET       if A is a valid tx then @addr to the tx timestamp
			return "Get_Timestamp_For_Tx_In_A";
		case OpCode.Get_Random_Id_For_Tx_In_A : // 0x0308; // EXT_FUN_RET       if A is a valid tx then @addr to the tx random id***
		...
		case OpCode.Get_Previous_Balance     : // 0x0401; // EXT_FUN_RET       sets @addr to the balance it had last had when running*
		...
		case OpCode.Send_All_To_Address_In_B : // 0x0403; // EXT_FUN           if B is a valid address then send it the entire balance
			return "Send_All_To_Address_In_B";
		case OpCode.Send_Old_To_Address_In_B : // 0x0404; // EXT_FUN           if B is a valid address then send it the old balance**
		...
		case OpCode.Add_Minutes_To_Timestamp : // 0x0406; // EXT_FUN_RET_DAT_2 set @addr1 to timestamp $addr2 plus $addr3 minutes***
