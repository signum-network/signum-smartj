...
public class EmulatorWindow extends JFrame implements ActionListener {
	...
	public EmulatorWindow(Class<?> c) {
		...
		try {
			...
			}catch (Exception e) { lafc =  Class.forName("javax.swing.plaf.nimbus.NimbusLookAndFeel");}
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == forgeButton) {
			...
			if(rs==null || rs.trim().length()==0) {
				JOptionPane.showMessageDialog(EmulatorWindow.this,
						"Invalid receiver address", "Error", JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			try {
				...
				JOptionPane.showMessageDialog(EmulatorWindow.this,
						"Could not parse the amount", "Error", JOptionPane.ERROR_MESSAGE);
				return;				
			}
			...
			if(creatorRS==null || creatorRS.trim().length()==0) {
				JOptionPane.showMessageDialog(EmulatorWindow.this,
						"Creator address is empty", "Error", JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			if(atAddrRS==null || atAddrRS.trim().length()==0) {
				JOptionPane.showMessageDialog(EmulatorWindow.this,
						"Contract address is empty", "Error", JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			try {
				...
				JOptionPane.showMessageDialog(EmulatorWindow.this,
						"Invalid activation fee", "Error", JOptionPane.ERROR_MESSAGE);
				return;				
			}
			...
			try {
				...
				JOptionPane.showMessageDialog(EmulatorWindow.this,
						"Contract instantiation: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
				return;				
			}
			...
			try {
				...
				final JDialog dlg = new JDialog(this, "Compiled Contract",
